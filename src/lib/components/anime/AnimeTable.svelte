<script lang="ts">
	...
	let { mode, animes, showScore = true, emptyMessage = 'No results.' }: Props = $props();
...
{:else}
	<div class="-mx-3 overflow-x-auto px-3">
		<table class={['w-full table-fixed border-collapse text-sm', tableMinWidth()].join(' ')}>
			<thead>
				<tr
					class="border-b border-white/[0.07] text-[0.68rem] font-semibold tracking-wide text-neutral-500 uppercase"
				>
					{#if mode === 'ranking'}
			...
			<tbody class="divide-y divide-white/[0.07]">
				{#each animes as anime, index (anime.id)}
					<tr class="group transition hover:bg-white/[0.045]">
						{#if mode === 'ranking'}
							<td class="px-3 py-1.5 align-middle text-xs text-neutral-500 tabular-nums">
								{getRank(anime, index)}
							</td>
						...
						{:else}
							<td class="px-3 py-1.5 align-middle text-xs text-neutral-600 tabular-nums">
						...
						<td class="w-[300px] min-w-[300px] px-3 py-1.5 align-middle">
							...
							>
								<h2
									class="truncate text-[0.78rem] leading-tight font-medium text-neutral-100 transition group-hover:text-white"
								>
									{anime.title}
								</h2>
						...
						{#if shouldShowScore()}
							<td
								class="px-3 py-1.5 text-left align-middle text-sm font-semibold text-accent tabular-nums"
							>
								{getScore(anime)}
							</td>
						{/if}
						...
						<td
							class="truncate px-3 py-1.5 text-right align-middle text-sm text-neutral-300 tabular-nums"
						>
							{formatEpisodes(anime)}
						</td>

						<td
							class="truncate px-3 py-1.5 text-right align-middle text-xs text-neutral-400 tabular-nums"
						>
