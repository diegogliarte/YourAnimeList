<script lang="ts">
	import type { RuntimeAnimeStat } from '$lib/utils/anime-stats';

	type Props = {
		title: string;
		items: RuntimeAnimeStat[];
		emptyMessage?: string;
	};

	let { title, items, emptyMessage = 'No runtime data.' }: Props = $props();
</script>
	...
	{:else}
		<table class="w-full text-left text-xs">
			...
			<tr>
				...
				<th class="px-3 py-1.5 text-right font-medium">eps</th>
			...
			{#each items as item (item.id)}
				<tr class="border-b border-white/5 last:border-b-0">
					<td class="w-full px-3 py-1.5 text-neutral-300">
							<span class="line-clamp-1" title={item.title}>
								{item.title}
							</span>
					</td>
